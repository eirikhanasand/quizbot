...
module.exports.run = async (bot, message, args) => {
  MongoClient.connect(url, { useUnifiedTopology: true }, async function(err, client) {
    ...
    const Gembed = new Discord.RichEmbed()
            .setDescription(`Global toppliste [Rangert etter antall feil svar]`)
            .setColor("#00ff00")
            .setFooter("Du ser nå den totale statistikkoversikten, rangert etter antall feil svar (!wronglb).\nPrøv  !leaderboard, !pointslb, !entrieslb, !streaklb, longestlb og !correctlb også.")
      ...
      const gtopdocs = await doc
        if (err) {console.log(err)}
          ...
          if (gtop < 1) {
            ...
            Gembed.addField(`${await gtopawaited.name}`, `${await gtopawaited.wrong}`)            
          }
