...
//  CONNECT TO MONGODB DATABASE
    MongoClient.connect(url, { useUnifiedTopology: true }, async function(err, client) {
        ...
        const delari = dbc.updateMany(
             { thisround: { $gt: 0 }},
             { $set: { "recentActivity" : "Updated", "thisround" : 0} }
             )
...
//      THIS IF ELSE CHAIN IS FOR RESETTING THE UNANSWERED COUNTER EACH TIME THE COMMAND IS CALLED.
        ...
        .setDescription(`Quizzen ble automatisk stoppet etter 5 spørsmål.\nHer er oversikten over spillet.`)
        .setColor("#F72C00")
...
//          IF THE LAST TWO QUESTIONS HAVE NOT BEEN ANSWERED, SAY THIS IN THE CHANNEL WHERE IT WAS USED, THEN STOP THE LOOP.
                    ...
                    await ari.each(async (err, doc) => {
                      ...
                      setTimeout(()=>message.channel.send(UEembed),100))
                    await dbc.updateMany(
                        {ari},
        ...
        }, setTimeout(() => message.channel.send(LEembed),100))
